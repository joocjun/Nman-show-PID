...
class PIDControllerExplicitMultiDof(object):

    def __init__(self, pb):
        self._pb = pb
        self.integral_errors = {}  # To store integral terms for each body
    ...
    def computePID(
        ...
        if bodyUniqueId not in self.integral_errors:
            self.integral_errors[bodyUniqueId] = {
                ...
                "last_time": self._pb.getPhysicsEngineParameters()["physicsTime"],
            }

        integral_data = self.integral_errors[bodyUniqueId]
        current_time = self._pb.getPhysicsEngineParameters()["physicsTime"]
        ...
        for i in range(numJoints):
            ...
            if len(js[0]) == 1:
                ...
                qError.append(qdiff)
                zeroAccelerations.append(0.0)
                qdot1 += jointVel
                qIndex += 1
                qdotIndex += 1
            if len(js[0]) == 4:
                desiredPos = [
                    desiredPositions[qIndex],
                    desiredPositions[qIndex + 1],
                    desiredPositions[qIndex + 2],
                ...
                qError.append(0)
                desiredVel = [
                    desiredVelocities[qdotIndex],
                    desiredVelocities[qdotIndex + 1],
        ...
        integral_data["position"] += np.array(qError) * dt
        ...
        p = Kp.dot(qError)
        i = Ki.dot(integral_data["position"])
        d = Kd.dot(qdoterr)
...
class PIDControllerExplicit(object):
    ...
    def computePID(
        ...
        # Update integral term
        integral_data["position"] += qError * dt
        ...
        forces = Kp.dot(qError) + Ki.dot(integral_data["position"]) + Kd.dot(qdotError)
        maxF = np.array(maxForces)
        forces = np.clip(forces, -maxF, maxF)
